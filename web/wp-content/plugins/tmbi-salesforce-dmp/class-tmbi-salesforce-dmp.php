...
class TMBI_Salesforce_DMP {
	...
	public static function init() {
		...
		add_action( 'wp_enqueue_scripts', array( __CLASS__, 'localize_krux_data_layer' ) );
		add_action( 'wp_footer', array( __CLASS__, 'remove_krux_tracking' ), 1 );
		add_filter( 'krux_categories', array( __CLASS__, 'categories_for_krux' ), 2 );
		add_filter( 'post_tags_krux', array( __CLASS__, 'get_post_tags' ) );
		add_filter( 'post_author_krux', array( __CLASS__, 'get_post_authors' ) );
		add_filter( 'gs_categories_krux', array( __CLASS__, 'gs_categories' ) );
		add_filter( 'post_taxonomies_krux', array( __CLASS__, 'get_post_taxonomy' ) );
	}

	/**
	 * Localize the Krux targeting data
	 */
	public static function localize_krux_data_layer() {
...
(function(){
  ...
  Krux.segments = retrieve('segs') ? retrieve('segs').split(',') : [];
	...
	public static function krux_build_datalayer() {
		...
		$krux_categories    = apply_filters( 'krux_categories', '', '' );
		$krux_tags          = apply_filters( 'post_tags_krux', '' );
		$krux_author        = apply_filters( 'post_author_krux', '' );
		$krux_gs_categories = apply_filters( 'gs_categories_krux', $content_id );
		$krux_taxonomies    = apply_filters( 'post_taxonomies_krux', '' );

		self::$krux_data_layer['page'] = array(
			'category' => $krux_categories[0],
			'topic'    => $krux_categories[1],
			'keyword'  => $krux_tags,
		);

		self::$krux_data_layer['page']['contentid']     = $content_id;
		self::$krux_data_layer['page']['author']        = $krux_author;
		self::$krux_data_layer['page']['gs_categories'] = $krux_gs_categories;

		$tax_array = $krux_taxonomies;
		if ( $tax_array ) {
	...
	/**
	 * The gs categories for Krux data layer.
	 *
	 * @param string $content_id post content ID.
	 *
	 * @return array|mixed|object
	 */
	public function gs_categories( $content_id = '' ) {
		$gs_categories = json_decode( get_post_meta( $content_id, 'gs_channels', true ) );
		return( $gs_categories );
	}

	/**
	 * Categories for Krux data layer.
	 *
	 * @param string $top_level_categories Parent categories.
	 * @param string $sub_categories Child categories.
	 *
	 * @return array
	 */
	public function categories_for_krux( $top_level_categories = '', $sub_categories = '' ) {

		$categories = self::get_category_data();
		if ( is_array( $categories ) ) {
			if ( array_key_exists( 'parent_categories', $categories ) ) {
				$top_level_categories = self::format_data( $categories['parent_categories'] );
			}
			if ( array_key_exists( 'sub_categories', $categories ) ) {
				$sub_categories = self::format_data( $categories['sub_categories'] );
			}
		}
		return array( $top_level_categories, $sub_categories );
	}

	/**
	 * Get the taxonomies of the posts
	 *
	 * @param array|string $taxonomies post taxonomies.
	 * @return array|string
	 */
	public static function get_post_taxonomy( $taxonomies ) {
		if ( ! is_single() ) {
			return '';
		}
	 ...
	 *
	 * @param array|string $authors post authors.
	 * @return array|string
	 */
	public static function get_post_authors( $authors ) {
		$author_list_format = '';
		$author_list        = tmbi_salesforce_get_the_authors();
		if ( $author_list ) {
	 ...
	 **/
	public static function get_post_tags( $tags ) {
