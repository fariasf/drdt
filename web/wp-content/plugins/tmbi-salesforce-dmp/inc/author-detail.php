 ...
 * @return array()
 */
function tmbi_salesforce_get_the_authors() {
	global $post;
	$post_id = $post->ID;
	...
	if ( function_exists( 'get_coauthors' ) ) {
		...
		foreach ( $authors as $author ) {
			$author_list[] = array(
				'author_image' => tmbi_salesforce_get_author_image(
					$author->ID,
					$author->type,
					array(
						'size'   => 75,
	...
	if ( count( $author_list ) === 0 ) {
		...
		$author_list[]        = array(
			'author_image' => tmbi_salesforce_get_author_image( get_the_author_meta( 'ID' ), 'wp_author', '75' ),
 ...
 *
 * @param array|string $author_id the author ID.
 * @param array|string $author_type the author type.
 * @param array|string $size the size.
 * @return string
 */
function tmbi_salesforce_get_author_image( $author_id, $author_type, $size ) {
