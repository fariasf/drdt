...
(function(){
  ...
  Krux.segments = retrieve('segs') ? retrieve('segs').split(',') : [];
		...
		echo $contol_tag . $interchage_tag ;
	}
	...
	public static function krux_build_datalayer() {
		...
		if ( is_array( $categories ) ) {
			if ( array_key_exists( 'parent_categories', $categories ) && ( class_exists( 'TMBI_Ad_Stack' ) ) ) {
				$top_level_categories = self::format_data( $categories['parent_categories'] );
			}
			if ( array_key_exists( 'sub_categories', $categories ) && ( class_exists( 'TMBI_Ad_Stack' ) ) ) {
		...
		self::$krux_data_layer['page']['contentid'] = $contentId;
		self::$krux_data_layer['page']['author']    = self::get_post_authors();
		self::$krux_data_layer['page']['gs_categories'] = json_decode( get_post_meta( $contentId, 'gs_channels' , true ) );
	...
	public static function get_post_taxonomy() {
		...
		$relevant_taxonomies = array_filter( $taxonomies, function( $taxonomy ) {
			return ! in_array( $taxonomy, self::$excluded_taxonomies );
		});
	...
	public static function get_post_authors() {
		$author_list_format = '';
		$author_list        = '';
		if ( class_exists( 'TMBI_Author_Detail' ) ) {
			$author_list = TMBI_Author_Detail::get_the_authors();
		}
		if ( $author_list ) {
