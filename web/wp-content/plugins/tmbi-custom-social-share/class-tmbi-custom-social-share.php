		...
		if ( is_single() || get_post_type( get_the_ID() ) === 'listicle' || is_archive() ) {
			...
			foreach ( self::$default_share_icons as $social_icons ) {
				...
				switch ( strtoupper( trim( $social_icons ) ) ) {
					case 'FACEBOOK':
						echo wp_kses_post( '<a class="pure-menu-link" data-analytics-metrics=' . self::tmbi_adobe_analytics( 'facebook', $position_prop ) . ' id="fb-share" onClick="window.open(\'https://www.facebook.com/sharer/sharer.php?u=' . rawurlencode( $page_url ) . '&t' . rawurlencode( $page_title ) . '=&v=3\',\'sharer\',\'toolbar=0,status=0,width=548,height=325\');" href="javascript: void(0)"><img class="social-icons" src="' . plugin_dir_url( __FILE__ ) . '/images/facebook-letter-logo.svg" alt="facebook" /></a>' );

						break;

					case 'TWITTER':
						echo wp_kses_post( '<a class="pure-menu-link" data-analytics-metrics=' . self::tmbi_adobe_analytics( 'twitter', $position_prop ) . ' id="twitter-share" onClick="window.open(\'https://twitter.com/share?url=' . rawurlencode( $page_url ) . '&amp;text=' . rawurlencode( $page_title ) . '&amp;hashtags=\',\'sharer\',\'toolbar=0,status=0,width=548,height=325\');" href="javascript: void(0)"><img class="social-icons" src="' . plugin_dir_url( __FILE__ ) . './images/twitter-logo.svg" alt="twitter" /></a>' );

						break;

					case 'PINTEREST':
						echo wp_kses_post( '<a class="pure-menu-link" data-analytics-metrics=' . self::tmbi_adobe_analytics( 'pinterest', $position_prop ) . ' id="pinterest-share" onClick="window.open(\'https://pinterest.com/pin/create/button/?url=' . rawurlencode( $page_url ) . '&media=' . rawurlencode( $page_image ) . '&description=' . rawurlencode( $page_title ) . ',\'sharer\',\'toolbar=0,status=0,width=548,height=325\');" href="javascript: void(0)"><img class="social-icons" src="' . plugin_dir_url( __FILE__ ) . './images/pinterest-social-visual-website-logotype.svg" alt="Pinterest" /></a>' );

						break;

					case 'EMAIL':
						echo wp_kses_post( '<li class="social-share-item"><a class="pure-menu-link" data-analytics-metrics=' . self::tmbi_adobe_analytics( 'email', $position_prop ) . '  id="email_a_friend" onClick="window.open(\'' . self::tmbi_custom_email() . '\',\'_self\')" href="javascript: void(0)" > <img class="social-icons" src="' . plugin_dir_url( __FILE__ ) . './images/envelope.svg" alt="Email" /></a></li> ' );

						break;

					case 'LINKEDIN':
						echo wp_kses_post( '<a class="pure-menu-link" target="_blank" id="linkedin-share" onClick="window.open(\'https://www.linkedin.com/shareArticle?mini=true&url=' . $page_url . '&title=' . $page_title . '&summary=' . $page_desc . '&source=\',\'sharer\',\'toolbar=0,status=0,width=548,height=325\');" href="javascript: void(0)"> <img class="social-icons" src="' . plugin_dir_url( __FILE__ ) . './images/linkedin.svg" alt="Linkedin" /></a> ' );

						break;

					case 'REDDIT':
						echo wp_kses_post( '<a class="pure-menu-link" target="_blank" id="reddit-share" onClick="window.open(\'http://reddit.com/submit?url=' . $page_url . '&amp;title=' . $page_title . '\',\'sharer\',\'toolbar=0,status=0,width=548,height=325\');" href="javascript: void(0)"> <img class="social-icons" src="' . plugin_dir_url( __FILE__ ) . './images/reddit.svg" alt="Reddit" /></a>' );
