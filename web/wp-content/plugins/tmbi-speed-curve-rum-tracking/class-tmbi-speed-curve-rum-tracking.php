...
class TMBI_SPEED_CURVE_RUM_TRACKING {
	...
	public function tmbi_speed_curve_script() {
		wp_register_script(
			self::SCRIPT_NAME,
			plugins_url( self::SCRIPT_FILE, __FILE__ ),
			array( self::DEPENDS ),
			self::VERSION,
			self::IN_FOOTER
		);
		...
		wp_register_script(
			self::CUSTOM_SCRIPT_SLUG,
			plugins_url( self::CUSTOM_SCRIPT_URL, __FILE__ ),
			array( 'jquery', self::SC_SCRIPT_SLUG, self::SCRIPT_NAME ),
			self::VERSION,
			self::IN_FOOTER
		);
